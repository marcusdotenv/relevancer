...
class Trie:
    ...
    def __term_to_nodes(self, term: str, original_term: str, frequency: int) -> Node:
        nodes = list(map(lambda it: Node(it), term))
        final_term_node = Node("*", original_term, frequency=frequency)
    ...
    def insert(self, search_term: str, frequency: int=0):
        ...
        accumulated_nodes = self.__term_to_nodes(term=lower_search_term, original_term=search_term, frequency=frequency)

        if self.__root.has_node_with_letter(letter=lower_search_term[0]):
            self.__assign_already_existing_path(search_term=lower_search_term, frequency=frequency, original_term=search_term)
        else:
    ...
    def __assign_already_existing_path(self, search_term: str, frequency: int, original_term: str):
        ...
        next_node.assign_new_node(Node("*", frequency=frequency, term=original_term))
    ...
    def __collect_terms_from_node_lazy(self, node: Node, prefix: str) -> list[tuple[str, int]]:
        ...
        if node.is_final_node():
            final_node = node.get_next_node_by_letter("*")
            collected_terms.append((final_node.term, final_node.get_frequency()))
